...
(() => {
  ...
  async function ensureUserProfile() {
    ...
    // If this Firebase account is listed in acharyas but the users profile
    // has not been promoted yet, treat it as an Acharya account.
    if (currentRole === "user") {
      try {
        const acharyaQuery = F().query(
          F().collection(DB(), "acharyas"),
          F().where("uid", "==", user.uid),
          F().limit(1)
        );
        const acharyaSnap = await F().getDocs(acharyaQuery);
        if (!acharyaSnap.empty) {
          currentRole = "acharya";
          currentProfile.role = "acharya";
          await F().setDoc(
            ref,
            { role: "acharya", updatedAt: F().serverTimestamp() },
            { merge: true }
          );
        }
      } catch (error) {
        console.warn("Acharya role lookup failed:", error);
      }
    }
  ...
  async function openConversation(conversation) {
    ...
    // Opening a conversation marks all messages in it as read for this account.
    try {
      await F().updateDoc(
        F().doc(DB(), "conversations", conversation.id),
        { unreadForUid: "", unreadCount: 0 }
      );
    } catch (error) {
      console.warn("Could not clear conversation unread state:", error);
    }
  ...
  async function sendChatMessage(event) {
    ...
    try {
      ...
      const recipientUid =
        user.uid === currentConversation.userId
          ? currentConversation.acharyaUid
          : currentConversation.userId;

      const conversationRef = F().doc(
        DB(),
        "conversations",
        currentConversation.id
      );
      const conversationSnap = await F().getDoc(conversationRef);
      const conversationData = conversationSnap.exists()
        ? conversationSnap.data()
        : {};
      const nextUnreadCount =
        conversationData.unreadForUid === recipientUid
          ? Number(conversationData.unreadCount || 0) + 1
          : 1;
  ...
  async function loadMessagesInbox() {
    ...
    currentConversationUnsubscribe = F().onSnapshot(
      ...
      (snap) => {
        ...
        updateMessageCount(conversations, user.uid);
        maybeNotifyIncomingMessages(conversations, user.uid);
      },
      (error) => {
  ...
  function updateMessageCount(conversations, uid = getAuthUser()?.uid) {
    const unreadCount = conversations.reduce((total, conversation) => {
      if (conversation.unreadForUid !== uid) return total;
      return total + Math.max(1, Number(conversation.unreadCount || 1));
    }, 0);
  ...
  function maybeNotifyIncomingMessages(conversations, uid) {
    if (!("Notification" in window) || Notification.permission !== "granted") return;

    conversations.forEach((conversation) => {
      if (conversation.unreadForUid !== uid || !conversation.lastAt) return;

      const lastAtMs = timestampMs(conversation.lastAt);
      if (!lastAtMs) return;

      const storageKey = `nakshatra-notified-${uid}-${conversation.id}`;
      const previous = Number(localStorage.getItem(storageKey) || 0);
      if (lastAtMs <= previous) return;

      localStorage.setItem(storageKey, String(lastAtMs));

      try {
        const title = conversation.acharyaName
          ? `नया संदेश — ${conversation.acharyaName}`
          : "Nakshatra Jyoti — नया संदेश";
        const notification = new Notification(title, {
          body: conversation.lastMessage || "आपको एक नया संदेश मिला है।",
