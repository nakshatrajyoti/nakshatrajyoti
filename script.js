    ...
    firebaseAuthModule.onAuthStateChanged(
      ...
      (user) => {
        ...
        if (user) {
          ...
          /* Restore the feature page/view only after Firebase auth is ready.
             This prevents the HTML default Call page from winning after refresh. */
          setTimeout(() => {
            try {
              restoreLastPage();
              restoreMessagesView();
            } catch (error) {
              console.warn("Saved page/view restore failed:", error);
            }
          }, 120);
...
(() => {
      ...
      <article class="acharya-detail" data-acharya-id="${safeText(a.id)}">
        ...
        <div class="acharya-detail-content">
          ...
          <div class="social-buttons acharya-social-links">
            <button class="contact-button" type="button" data-acharya-contact="${safeText(a.id)}">☎ संपर्क</button>
            <button class="instagram-button" type="button" data-acharya-instagram="${safeText(a.id)}">◎ Instagram</button>
            <button class="facebook-button" type="button" data-acharya-facebook="${safeText(a.id)}">f Facebook</button>
          </div>
        </div>
      </article>
    `).join("");

    list.querySelectorAll("[data-acharya-contact]").forEach((button) => {
      button.addEventListener("click", async () => {
        const a = await getAcharyaById(button.dataset.acharyaContact);
        if (a?.phone) {
          window.location.href = `tel:${a.phone}`;
        } else {
          showFeatureToast(`${a?.name || "आचार्य"} का संपर्क नंबर अभी प्रोफ़ाइल में नहीं जोड़ा गया है।`);
        }
      });
    });

    list.querySelectorAll("[data-acharya-instagram]").forEach((button) => {
      button.addEventListener("click", async () => {
        const a = await getAcharyaById(button.dataset.acharyaInstagram);
        if (a?.instagram) {
          window.open(a.instagram, "_blank", "noopener,noreferrer");
        } else {
          showFeatureToast(`${a?.name || "आचार्य"} का Instagram अभी प्रोफ़ाइल में नहीं जोड़ा गया है।`);
        }
      });
    });

    list.querySelectorAll("[data-acharya-facebook]").forEach((button) => {
      button.addEventListener("click", async () => {
        const a = await getAcharyaById(button.dataset.acharyaFacebook);
        if (a?.facebook) {
          window.open(a.facebook, "_blank", "noopener,noreferrer");
        } else {
          showFeatureToast(`${a?.name || "आचार्य"} का Facebook अभी प्रोफ़ाइल में नहीं जोड़ा गया है।`);
        }
      });
    });
  }
  ...
  /* =========================================================
     MESSAGE / AI VIEW STATE
     Keep the professional Messages and AI workspaces stable
     across refreshes. The Call page must never overwrite them.
  ========================================================= */

  function saveMessagesView(view) {
    const allowed = ["inbox", "ai"];
    if (!allowed.includes(view)) return;
    localStorage.setItem("nakshatra-messages-view", view);
  }
  ...
  function restoreMessagesView() {
    ...
    const view = localStorage.getItem("nakshatra-messages-view") || "inbox";
  ...
  function showChatWorkspace(saveState = true) {
    $("messagesList")?.classList.add("hidden");
    $("chatWorkspace")?.classList.remove("hidden");
    $("aiChatWorkspace")?.classList.add("hidden");
    if (saveState) saveMessagesView("inbox");
  }

  function showMessagesInbox(saveState = true) {
  ...
  function showAIWorkspace(saveState = true) {
    $("messagesList")?.classList.add("hidden");
    $("chatWorkspace")?.classList.add("hidden");
    $("aiChatWorkspace")?.classList.remove("hidden");
    if (saveState) saveMessagesView("ai");
    renderAIChat();
  }
  ...
  async function initFeatureLayer() {
    ...
    /* Final route restore after the feature layer has finished mounting. */
    setTimeout(() => {
      try {
        restoreLastPage();
        restoreMessagesView();
      } catch (error) {
        console.warn("Final route restore failed:", error);
      }
    }, 80);
  }
