...
(() => {
  ...
  // A single failing step here (a rules error, a missing Firestore
  // index, a network blip) must never prevent every step after it
  // from running — that was the actual bug: one rejected await inside
  // a single try/catch silently stopped the whole rest of the chain
  // (Poster, Rashifal, Notifications, Acharya list, Messages — every
  // one of them sits downstream of some earlier step in this list).
  // This wrapper isolates each step so they succeed or fail
  // independently; nothing about what each step itself does changes.
  async function safeInitStep(label, fn) {
    try {
      await fn();
    } catch (error) {
      console.warn(`Init step "${label}" failed — continuing with the rest of startup:`, error);
    }
  }
  ...
  async function ensureRoleAndProfile() {
    try {
      ...
      await safeInitStep("ensureUserProfile", ensureUserProfile);

      // The admins/{uid} document is the authoritative admin marker.
      // It is checked before the normal users/{uid}.role value.
      let isAdmin = false;
      await safeInitStep("resolveAdminRole", async () => {
        isAdmin = await resolveAdminRole();
      });
      ...
      if (currentRole === "admin") {
        await safeInitStep("syncAcharyaDefaults", syncAcharyaDefaults);
      }

      await safeInitStep("loadBlog", loadBlog);
      await safeInitStep("loadSiteSettings", loadSiteSettings);
      await safeInitStep("startSiteSettingsRealtime", startSiteSettingsRealtime);
      await safeInitStep("wireRashifalGrid", wireRashifalGrid);
      await safeInitStep("startRashifalRealtime", startRashifalRealtime);
      await safeInitStep("startHomeFeedRealtime", startHomeFeedRealtime);
      await safeInitStep("wireFeedPillAndLoadMore", wireFeedPillAndLoadMore);
      await safeInitStep("startNotificationCenterRealtime", startNotificationCenterRealtime);
      await safeInitStep("startPresenceHeartbeat", startPresenceHeartbeat);
      await safeInitStep("startPresenceRealtime", startPresenceRealtime);
      await safeInitStep("startAcharyasRealtime", startAcharyasRealtime);
      await safeInitStep("loadMessagesInbox", loadMessagesInbox);
      await safeInitStep("renderAdminStats", renderAdminStats);
      await safeInitStep("renderAdminPosts", renderAdminPosts);
      await safeInitStep("renderAdminAcharyas", renderAdminAcharyas);
  ...
  function wirePostMediaTypeToggle() {
    ...
    const apply = () => {
      ...
      $("postPhotoFieldWrap").hidden = isVideo;
      $("postVideoFieldWrap").hidden = !isVideo;
    };
  ...
  function wirePostVideoUpload() {
    const button = $("postVideoChooseButton");
    const input = $("postVideoFileInput");
  ...
  async function renderAcharyaOwnProfileForm() {
    const box = $("acharyaOwnProfileBox");
    ...
    // forms, etc.).
    const form = $("acharyaOwnProfileForm");
    form.addEventListener("submit", async (event) => {
      event.preventDefault();
      const status = $("acharyaOwnProfileStatus");
      const id = $("acharyaOwnProfileId").value;

      const payload = {
        name: $("acharyaOwnProfileName").value.trim(),
        qualification: $("acharyaOwnProfileQualification").value.trim(),
        speciality: $("acharyaOwnProfileSpeciality").value.trim(),
        bio: $("acharyaOwnProfileBio").value.trim(),
        image: isSafeMediaUrl($("acharyaOwnProfileImage").value.trim())
          ? $("acharyaOwnProfileImage").value.trim()
          : mine.image || ""
      };
  ...
  function openRashifalDetail(signId) {
    ...
    $("rashifalDetailSymbol").textContent = rashi.symbol;
    $("rashifalDetailName").textContent = rashi.name;
    $("rashifalDetailDate").textContent = item?.date || todayDateLabel();
    $("rashifalDetailText").textContent = item?.text?.trim()
      ? item.text
      : "इस राशि के लिए आज का राशिफल अभी उपलब्ध नहीं है। कृपया बाद में देखें।";
    $("rashifalDetailMeta").textContent = item?.updatedBy
      ? `— ${item.updatedBy} द्वारा अपडेट`
      : "";

    $("rashifalDetailOverlay")?.classList.add("show");
  }

  function closeRashifalDetail() {
    $("rashifalDetailOverlay")?.classList.remove("show");
  }
  ...
  function wireRashifalGrid() {
    ...
    const overlay = $("rashifalDetailOverlay");
    if (overlay && overlay.dataset.wired !== "true") {
      overlay.dataset.wired = "true";
      $("rashifalDetailClose")?.addEventListener("click", closeRashifalDetail);
  ...
  function showFeedNewPostsPill() {
    const pill = $("feedNewPostsPill");
    const countEl = $("feedNewPostsCount");
  ...
  function flushFeedPendingNew() {
    ...
    $("homeFeed")?.scrollIntoView({ behavior: "smooth", block: "start" });
  }
  ...
  function startHomeFeedRealtime() {
    ...
    // REQUIRED FIRESTORE INDEX (this query will fail without it):
    //   Collection ID:  posts
    //   Fields indexed: published  Ascending
    //                   createdAt  Descending
    //   Query scope:    Collection
    // Same requirement as the notifications index documented in
    // startNotificationCenterRealtime() below — Firestore needs a
    // composite index any time a where() and an orderBy() on a
    // different field are combined. Firebase Console → Firestore
    // Database → Indexes → Composite → Create Index with the two
    // fields above, in that order (or click the direct link Firebase
    // prints in the browser console the first time this query runs).
    const q = F().query(
    ...
    F().onSnapshot(
      ...
      (snap) => {
        if (!initialLoadDone) {
          ...
          $("feedLoadOlder").hidden = !feedHasMoreOlder;
          renderInitialFeed();
          wireFeedCardEvents();
  ...
  function startNotificationCenterRealtime() {
    ...
    //   Fields indexed: targetUid  Ascending
    //                   createdAt  Descending
    //   Query scope:    Collection
    // Firebase Console → Firestore Database → Indexes → Composite →
    // Create Index, with exactly those two fields in that order. The
    // first time this query runs in a real browser, the console error
    // also contains a direct "create index" link that fills this in
    // automatically — click it once and it's done from then on.
    // Until the index exists/builds, this listener's error callback
    // below fires (visible in devtools as "FirebaseError: The query
    // requires an index...") — it no longer blocks Poster/Rashifal/
    // Acharya/Messages from loading (see safeInitStep in
    // ensureRoleAndProfile), only the Notification Center itself stays
    // empty until the index is created.
    const q = F().query(
  ...
  async function openConversation(conversation) {
    ...
    const subtitleText = $("chatSubtitleText") || subtitle;
    if (subtitleText) {
  ...
  function resetPostForm() {
    ...
    if ($("postVideoUrl")) $("postVideoUrl").value = "";
    if ($("postVideoStatus")) $("postVideoStatus").textContent = "";
    const preview = $("postVideoPreview");
    if (preview) { preview.hidden = true; preview.removeAttribute("src"); }
    if ($("postMediaType")) {
      $("postMediaType").value = "photo";
      $("postMediaType").dispatchEvent(new Event("change"));
    }
  }
