...
function openPage(
  ...
  // Acharya doesn't get the normal "consultation cards" call list
  // (that shows every acharya, including themselves) — their
  // बॉटम नेव "कॉल" बटन उनकी अपनी Messages inbox खोलता है
  // (users जिन्होंने उन्हें message किया)।
  if (pageName === "call" && window.NJCurrentRole === "acharya") {
    pageName = "messages";
  }
...
(() => {
  ...
  async function ensureRoleAndProfile() {
    try {
      ...
      if (!user) {
        currentProfile = null;
        currentRole = "user";
        window.NJCurrentRole = currentRole;
        refreshRoleUI();
    ...
    if (currentRole === "admin") {
      page.innerHTML = `
        ...
        <section class="role-panel">
          ...
          <p class="role-muted">सभी categories के user requests यहाँ दिखेंगी — कोई भी आचार्य/admin जवाब दे सकता है।</p>
          <div id="acharyaGuidanceList" class="admin-post-list"></div>
        </section>

        <section class="role-panel">
          <div class="section-label">SUPER ADMIN • BROADCAST</div>
          <h2>📢 सूचनाएँ</h2>
          <p class="role-muted">यहाँ से भेजी सूचना सभी users को notification center में दिखेगी।</p>
          <form id="broadcastForm" class="admin-post-form">
            <label>शीर्षक<input id="broadcastTitle" type="text" maxlength="120" required placeholder="विशेष सूचना"></label>
            <label>संदेश<textarea id="broadcastMessage" maxlength="400" required placeholder="आज शाम नई जानकारी उपलब्ध होगी।"></textarea></label>
            <button class="primary-button" type="submit">Publish करें</button>
            <div id="broadcastStatus" class="form-status"></div>
          </form>
        </section>
      `;
      return;
    ...
    if (currentRole === "acharya") {
      page.innerHTML = `
        ...
        <section class="role-panel">
          <div class="section-label">GUIDANCE</div>
          <h2>📋 Guidance Requests</h2>
          <p class="role-muted">users के भेजे प्रश्न — किसी का भी जवाब दें।</p>
          <div id="acharyaGuidanceList" class="admin-post-list"></div>
        </section>
  ...
  function wireRoleDashboardForms() {
    ...
    if (broadcastForm && broadcastForm.dataset.broadcastWired !== "true") {
      broadcastForm.dataset.broadcastWired = "true";
      broadcastForm.addEventListener("submit", publishAdminBroadcast);
    }
  }
  ...
  function refreshRoleDashboard() {
    ...
    if (currentRole === "admin" || currentRole === "acharya") {
      ...
      loadAllOpenGuidanceRequestsIntoDashboard();
    }
    ...
    list.innerHTML = acharyas.map((a) => `
      <article class="acharya-detail" data-acharya-id="${safeText(a.id)}">
        ...
        <div class="acharya-detail-content">
          ...
          <div class="acharya-posts-feed" data-acharya-posts="${safeText(a.id)}">
            <div class="admin-empty">विचार लोड हो रहे हैं…</div>
          </div>
        </div>
      </article>
    `).join("");

    await ensureMyLikedPostIds();
    acharyas.forEach((a) => renderAcharyaPostsFeed(a));
  ...
  async function renderAcharyaPostsFeed(acharya) {
    const box = document.querySelector(`[data-acharya-posts="${acharya.id}"]`);
    if (!box || !firestoreReady()) return;

    const uid = acharya.uid;
    if (!uid) {
      box.innerHTML = "";
      return;
    }
    ...
    try {
      ...
      acharyaPostsCache = [
        ...acharyaPostsCache.filter((p) => p.authorUid !== uid),
        ...posts
      ];

      if (!posts.length) {
        box.innerHTML = `<div class="admin-empty">अभी कोई विचार प्रकाशित नहीं हुआ।</div>`;
        return;
      }

      box.innerHTML = `
        <div class="section-label">आचार्य के विचार</div>
        ${posts.map((post) => `
          <article class="acharya-feed-post">
            ${postMediaHTML(post)}
            <div class="acharya-feed-post-body">
              <p class="acharya-feed-caption">${safeText(post.excerpt || truncateText(post.content, 160))}</p>
              <time>${safeText(formatFeatureDate(post.createdAt))}</time>
              ${postEngagementHTML(post)}
            </div>
          </article>
        `).join("")}
      `;
    } catch (error) {
      console.error("Load acharya posts feed error:", error);
      box.innerHTML = `<div class="admin-empty">विचार लोड नहीं हो सके।</div>`;
    }
  }
  ...
  async function openAcharyaChat(acharya) {
    ...
    if (acharya?.uid && acharya.uid === user.uid) {
      showFeatureToast("आप खुद को संदेश नहीं भेज सकते।");
      return;
    }
  ...
  async function openConversation(conversation) {
    ...
    const partnerUid = isStaff ? conversation.userId : conversation.acharyaUid;
    watchPartnerPresence(partnerUid);
  ...
  function chatMediaHTML(m) {
    ...
    if (m.mediaType === "image") {
      return `<img class="chat-media-image" src="${safeText(m.mediaUrl)}" alt="फोटो">`;
    }
    if (m.mediaType === "video") {
      return `<video class="chat-media-video" src="${safeText(m.mediaUrl)}" controls preload="metadata"></video>`;
    }
    return `<a class="chat-media-file" href="${safeText(m.mediaUrl)}" target="_blank" rel="noopener noreferrer">📄 ${safeText(m.fileName || "फाइल देखें")}</a>`;
  }
  ...
  async function sendChatMedia(file, kind) {
    const user = getAuthUser();
    if (!file || !user || !currentConversation || !firestoreReady()) return;

    if (!firebaseStorage || !firebaseStorageModule) {
      showFeatureToast("Storage तैयार नहीं है, फिर कोशिश करें।");
      return;
    }

    const maxMb = kind === "video" ? 60 : kind === "image" ? 15 : 25;
    if (file.size > maxMb * 1024 * 1024) {
      showFeatureToast(`फाइल ${maxMb}MB से छोटी होनी चाहिए।`);
      return;
    }

    const status = $("chatUploadStatus");
    if (status) status.textContent = "अपलोड हो रहा है…";

    try {
      const ext = (file.name.split(".").pop() || "dat").toLowerCase();
      const path = `uploads/${user.uid}/chat/${currentConversation.id}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
      ...
      const previewLabel = kind === "image" ? "📷 फोटो" : kind === "video" ? "🎥 Video" : "📄 फाइल";
      await F().updateDoc(
        F().doc(DB(), "conversations", currentConversation.id),
        {
      ...
      if (status) status.textContent = "";
      showFeatureToast("फाइल भेजी नहीं जा सकी।");
    }
  }
  ...
  let myLikedPostIds = [];
  let myLikedPostIdsLoaded = false;

  async function ensureMyLikedPostIds() {
    const user = getAuthUser();
    if (!user || !firestoreReady() || myLikedPostIdsLoaded) return myLikedPostIds;
    try {
      const snap = await F().getDoc(F().doc(DB(), "postLikes", user.uid));
      myLikedPostIds = snap.exists() ? (snap.data().ids || []) : [];
      myLikedPostIdsLoaded = true;
    } catch (error) {
      console.error("Load my likes error:", error);
    }
    return myLikedPostIds;
  }

  function postEngagementHTML(post) {
    const liked = myLikedPostIds.includes(post.id);
    return `
      <div class="post-engagement-bar">
        <button type="button" class="engage-button ${liked ? "liked" : ""}" data-like-post="${safeText(post.id)}">
          ${liked ? "❤️" : "🤍"} <span>${post.likeCount || 0}</span>
        </button>
        <button type="button" class="engage-button" data-comment-post="${safeText(post.id)}">
          💬 <span>${post.commentCount || 0}</span>
        </button>
        <button type="button" class="engage-button" data-share-post="${safeText(post.id)}">↗ Share</button>
      </div>`;
  }
  ...
  async function toggleLikePost(postId) {
    ...
    await ensureMyLikedPostIds();
    const liked = myLikedPostIds.includes(postId);

    try {
      const readsRef = F().doc(DB(), "postLikes", user.uid);
      const postRef = F().doc(DB(), "posts", postId);

      if (liked) {
        myLikedPostIds = myLikedPostIds.filter((id) => id !== postId);
        await F().setDoc(readsRef, { ids: myLikedPostIds }, { merge: true });
        await F().updateDoc(postRef, { likeCount: F().increment(-1) });
      } else {
        myLikedPostIds = [...myLikedPostIds, postId];
        await F().setDoc(readsRef, { ids: myLikedPostIds }, { merge: true });
        await F().updateDoc(postRef, { likeCount: F().increment(1) });
      }

      document.querySelectorAll(`[data-like-post="${postId}"]`).forEach((btn) => {
        const post = blogCache.find((p) => p.id === postId) || {};
        const newCount = (post.likeCount || 0) + (liked ? -1 : 1);
        post.likeCount = newCount;
        btn.classList.toggle("liked", !liked);
        btn.innerHTML = `${!liked ? "❤️" : "🤍"} <span>${Math.max(0, newCount)}</span>`;
      });
    } catch (error) {
  ...
  async function openCommentsModal(postId) {
    ...
    modal.className = "blog-modal-overlay comments-modal-overlay";
    modal.innerHTML = `
      <article class="blog-modal comments-modal">
        <button class="blog-modal-close" type="button">×</button>
        <h2>💬 Comments</h2>
        <div id="commentsList" class="comments-list"><div class="admin-empty">लोड हो रहा है…</div></div>
        ${user ? `
          <form id="addCommentForm" class="admin-post-form">
            <textarea id="newCommentText" maxlength="400" required placeholder="अपनी टिप्पणी लिखें…"></textarea>
            <button class="primary-button" type="submit">भेजें</button>
          </form>
        ` : `<p class="role-muted">टिप्पणी करने के लिए login करें।</p>`}
      </article>`;
    ...
    try {
      const snap = await F().getDocs(F().collection(DB(), "posts", postId, "comments"));
      ...
      list.innerHTML = comments.length
        ? comments.map((c) => `
            <div class="comment-item">
              <strong>${safeText(c.userName || "User")}</strong>
              <p>${safeText(c.text)}</p>
              <time>${safeText(formatFeatureDate(c.createdAt))}</time>
            </div>`).join("")
        : `<div class="admin-empty">अभी कोई comment नहीं है।</div>`;
    } catch (error) {
      console.error("Load comments error:", error);
    }

    modal.querySelector("#addCommentForm")?.addEventListener("submit", async (event) => {
      event.preventDefault();
      ...
      try {
        ...
        await F().updateDoc(F().doc(DB(), "posts", postId), { commentCount: F().increment(1) });

        document.querySelectorAll(`[data-comment-post="${postId}"] span`).forEach((span) => {
  ...
  function sharePost(post) {
    const shareData = {
      title: post.title || "नक्षत्र ज्योति",
      text: post.excerpt || truncateText(post.content, 120),
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(text).then(() => showFeatureToast("लिंक कॉपी हो गया।"));
    }
  }

  document.addEventListener("click", (event) => {
    const likeBtn = event.target.closest("[data-like-post]");
    if (likeBtn) { toggleLikePost(likeBtn.dataset.likePost); return; }

    const commentBtn = event.target.closest("[data-comment-post]");
    if (commentBtn) { openCommentsModal(commentBtn.dataset.commentPost); return; }

    const shareBtn = event.target.closest("[data-share-post]");
    if (shareBtn) {
      const post = blogCache.find((p) => p.id === shareBtn.dataset.sharePost) ||
        acharyaPostsCache.find((p) => p.id === shareBtn.dataset.sharePost);
  ...
  async function renderBlogList(source = blogCache) {
    const list = $("blogList");
    if (!list) return;
  ...
  function wireEvents() {
    ...
    document.querySelectorAll("[data-chat-attach]").forEach((button) => {
      button.addEventListener("click", () => {
    ...
    $("chatFileInput")?.addEventListener("change", (event) => {
      ...
      const kind = event.target.dataset.kind || "file";
      if (file) sendChatMedia(file, kind);
      event.target.value = "";
    });
  ...
  /* =========================================================
     ...
     Users submit a question from any service page (career,
     marriage, muhurat, education, wealth, family, health,
     travel). Any Acharya can see + answer it from their
     dashboard's Guidance panel.
  ========================================================= */

  const GUIDANCE_CATEGORY_LABELS = {
    career: "करियर",
    marriage: "विवाह",
    muhurat: "मुहूर्त",
    education: "शिक्षा",
  ...
  function guidanceCategoryFromPageId(pageId) {
    const map = {
      careerPage: "career",
      marriagePage: "marriage",
      muhuratPage: "muhurat",
  ...
  function injectGuidanceForms() {
    document.querySelectorAll(".service-page").forEach((page) => {
      if (page.querySelector(".guidance-request-box")) return;
      const category = guidanceCategoryFromPageId(page.id);
      if (!category) return;

      const label = GUIDANCE_CATEGORY_LABELS[category];
      const box = document.createElement("section");
      box.className = "guidance-request-box";
      box.innerHTML = `
        <div class="section-label">आपका प्रश्न</div>
        <h3>${safeText(label)} से जुड़ी अपनी समस्या लिखें</h3>
        <p class="role-muted">आपका प्रश्न हमारे आचार्यों के पास जाएगा और वे यहीं जवाब देंगे।</p>
        <form class="guidance-request-form" data-category="${category}">
          <textarea maxlength="600" required placeholder="अपना प्रश्न यहाँ लिखें…"></textarea>
          <button class="primary-button" type="submit">प्रश्न भेजें</button>
          <div class="guidance-request-status form-status"></div>
        </form>
        <div class="guidance-my-requests" data-my-requests="${category}"></div>
      `;
      page.appendChild(box);

      box.querySelector(".guidance-request-form").addEventListener("submit", submitGuidanceRequest);
    });
  }

  async function submitGuidanceRequest(event) {
    event.preventDefault();
    const form = event.currentTarget;
    const status = form.querySelector(".guidance-request-status");
    ...
    if (message.length < 8) {
      if (status) status.textContent = "कृपया अपना प्रश्न थोड़ा विस्तार से लिखें।";
      return;
    }

    try {
      await F().addDoc(F().collection(DB(), "guidanceRequests"), {
        category,
        message,
        userUid: user.uid,
        userName: user.displayName || user.email?.split("@")[0] || currentProfile?.name || "User",
      ...
      textarea.value = "";
      if (status) status.textContent = "आपका प्रश्न भेज दिया गया है — आचार्य जल्द जवाब देंगे।";
      loadMyGuidanceRequests(category);
    } catch (error) {
      console.error("Guidance submit error:", error);
      if (status) status.textContent = "प्रश्न भेजा नहीं जा सका। फिर कोशिश करें।";
    }
  }

  async function loadMyGuidanceRequests(category) {
    const box = document.querySelector(`[data-my-requests="${category}"]`);
    const user = getAuthUser();
    if (!box || !user || !firestoreReady()) return;

    try {
      const q = F().query(
        F().collection(DB(), "guidanceRequests"),
        F().where("userUid", "==", user.uid)
      );
      const snap = await F().getDocs(q);
      ...
      box.innerHTML = `
        <h4>आपके भेजे प्रश्न</h4>
        ${mine.map((r) => `
          <div class="guidance-request-card ${r.status === "answered" ? "answered" : "pending"}">
            <p class="guidance-question">${safeText(r.message)}</p>
            ${r.status === "answered"
              ? `<div class="guidance-answer"><strong>आचार्य ${safeText(r.answeredByName || "")} का जवाब:</strong><p>${safeText(r.answer)}</p></div>`
              : `<small class="guidance-pending-tag">⏳ जवाब की प्रतीक्षा है</small>`}
          </div>
        `).join("")}
      `;
    } catch (error) {
      console.error("Load my guidance error:", error);
    }
  }

  async function loadAllOpenGuidanceRequestsIntoDashboard() {
    const box = $("acharyaGuidanceList");
    if (!box || !firestoreReady()) return;

    box.innerHTML = `<div class="admin-empty">लोड हो रहा है…</div>`;

    try {
      const q = F().query(F().collection(DB(), "guidanceRequests"), F().limit(300));
      const snap = await F().getDocs(q);
      const all = snap.docs
        .map((d) => ({ id: d.id, ...d.data() }))
        .sort((a, b) => timestampMs(b.createdAt) - timestampMs(a.createdAt));
      ...
      if (!all.length) {
        box.innerHTML = `<div class="admin-empty">अभी कोई guidance request नहीं है।</div>`;
        return;
      }

      box.innerHTML = all.map((r) => `
        <article class="guidance-admin-card ${r.status === "answered" ? "answered" : ""}" data-guidance-id="${safeText(r.id)}">
          <div class="guidance-admin-head">
            <span class="guidance-category-tag">${safeText(GUIDANCE_CATEGORY_LABELS[r.category] || r.category)}</span>
            <strong>${safeText(r.userName)}</strong>
            <time>${safeText(formatFeatureDate(r.createdAt))}</time>
          </div>
          <p class="guidance-question">${safeText(r.message)}</p>
          ${r.status === "answered"
            ? `<div class="guidance-answer"><strong>जवाब (${safeText(r.answeredByName || "")}):</strong><p>${safeText(r.answer)}</p></div>`
            : `
              <form class="guidance-answer-form" data-guidance-answer="${safeText(r.id)}">
                <textarea maxlength="800" required placeholder="अपना जवाब लिखें…"></textarea>
                <button class="primary-button" type="submit">जवाब भेजें</button>
              </form>
            `}
        </article>
      `).join("");

      box.querySelectorAll(".guidance-answer-form").forEach((form) => {
        form.addEventListener("submit", answerGuidanceRequest);
      });
    } catch (error) {
      console.error("Load guidance dashboard error:", error);
      box.innerHTML = `<div class="admin-empty">Guidance requests लोड नहीं हो सके।</div>`;
    }
  }

  async function answerGuidanceRequest(event) {
    event.preventDefault();
    if (!roleCanManageContent() || !firestoreReady()) return;

    const form = event.currentTarget;
    const id = form.dataset.guidanceAnswer;
    ...
    try {
      const requestRef = F().doc(DB(), "guidanceRequests", id);
      const snap = await F().getDoc(requestRef);
      const data = snap.data();

      await F().updateDoc(requestRef, {
        status: "answered",
        answer,
        answeredByUid: user.uid,
        answeredByName: user.displayName || user.email?.split("@")[0] || "आचार्य",
        answeredAt: F().serverTimestamp()
      });

      if (data?.userUid) {
        await createNotification({
          audience: data.userUid,
          type: "guidance_answer",
          title: "आपके प्रश्न का जवाब आया",
          message: answer.slice(0, 140)
        });
      }

      showFeatureToast("जवाब भेज दिया गया।");
      loadAllOpenGuidanceRequestsIntoDashboard();
    } catch (error) {
      console.error("Answer guidance error:", error);
      showFeatureToast("जवाब भेजा नहीं जा सका।");
    }
  }
  ...
  /* =========================================================
     NOTIFICATION SYSTEM
     One "notifications" doc per event; audience is either a
     specific uid, or "all" / "acharyas" for broadcasts.
     Per-user read state lives in notificationReads/{uid}.ids
     (array of read notification ids) so a broadcast doc is
     still just one document regardless of audience size.
  ========================================================= */
  ...
  async function createNotification({ audience, type, title, message, meta = {} }) {
    if (!firestoreReady()) return;
    try {
      await F().addDoc(F().collection(DB(), "notifications"), {
        audience,
        type,
        title,
        message,
  ...
  function notificationAudienceList() {
    const user = getAuthUser();
    const list = ["all"];
    if (currentRole === "acharya") list.push("acharyas");
    if (user?.uid) list.push(user.uid);
  ...
  function startNotificationListener() {
    ...
    F().getDoc(F().doc(DB(), "notificationReads", user.uid))
      .then((snap) => {
        notificationReadIds = snap.exists() ? (snap.data().ids || []) : [];
    ...
    const q = F().query(
      ...
      F().where("audience", "in", notificationAudienceList()),
      F().limit(60)
    );

    notificationsUnsubscribe = F().onSnapshot(
      q,
      (snap) => {
        notificationCache = snap.docs
          .map((d) => ({ id: d.id, ...d.data() }))
          .sort((a, b) => timestampMs(b.createdAt) - timestampMs(a.createdAt));
        renderNotificationBadge();
  ...
  function renderNotificationBadge() {
    ...
    if (!badge) return;
    const unread = notificationCache.filter((n) => !notificationReadIds.includes(n.id)).length;
    if (unread > 0) {
  ...
  function renderNotificationPanel() {
    ...
    if (!notificationCache.length) {
      list.innerHTML = `<div class="notification-empty"><div>🔔</div><p>अभी कोई सूचना नहीं है।</p></div>`;
      return;
    }

    list.innerHTML = notificationCache.map((n) => `
      <div class="notification-item ${notificationReadIds.includes(n.id) ? "" : "unread"}">
        <div class="notification-icon">${notificationTypeIcon(n.type)}</div>
        <div class="notification-copy">
          <strong>${safeText(n.title)}</strong>
          <p>${safeText(n.message)}</p>
          <time>${safeText(formatFeatureDate(n.createdAt))}</time>
        </div>
      </div>
    `).join("");
  }
  ...
  async function markAllNotificationsRead() {
    const user = getAuthUser();
    if (!user || !firestoreReady() || !notificationCache.length) return;

    const newIds = notificationCache.map((n) => n.id).filter((id) => !notificationReadIds.includes(id));
    if (!newIds.length) return;

    notificationReadIds = [...notificationReadIds, ...newIds];
    renderNotificationBadge();
    renderNotificationPanel();

    try {
      await F().setDoc(
        F().doc(DB(), "notificationReads", user.uid),
        { ids: notificationReadIds.slice(-300) },
        { merge: true }
      );
    } catch (error) {
  ...
  async function publishAdminBroadcast(event) {
    event.preventDefault();
    if (!roleCanManageAdmin()) return;
    ...
    try {
      await createNotification({ audience: "all", type: "admin_broadcast", title, message });
      $("broadcastTitle").value = "";
      $("broadcastMessage").value = "";
      if (status) status.textContent = "सूचना publish हो गई।";
    } catch (error) {
      console.error("Broadcast error:", error);
      if (status) status.textContent = "publish नहीं हो सका।";
    }
  }
  ...
  let watchedPresenceUnsubscribe = null;
  ...
  function startPresenceHeartbeat() {
    ...
    presenceHeartbeatTimer = setInterval(beat, 30000);
  ...
  function watchPartnerPresence(uid) {
    if (watchedPresenceUnsubscribe) {
      watchedPresenceUnsubscribe();
      watchedPresenceUnsubscribe = null;
    }
    if (!uid || !firestoreReady()) return;

    watchedPresenceUnsubscribe = F().onSnapshot(
      F().doc(DB(), "presence", uid),
      (snap) => {
        const lastSeen = snap.data()?.lastSeen;
        const ms = timestampMs(lastSeen);
        const online = ms && (Date.now() - ms) < 90000;
        const dot = $("chatPresenceDot");
        const label = $("chatPresenceLabel");
        if (dot) dot.classList.toggle("online", !!online);
        if (label) label.textContent = online ? "ऑनलाइन" : "ऑफलाइन";
      },
      () => {}
    );
  }
