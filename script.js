...
/* =========================================================
   EARLY LANGUAGE BOOTSTRAP — MUST RUN BEFORE OTHER FEATURES
   This keeps the first screen usable even if a later feature
   encounters a runtime/network error.
========================================================= */
(function earlyLanguageBootstrap(){
  try {
    const continueButton = document.getElementById("languageContinue");
    if (!continueButton) return;

    continueButton.addEventListener("click", function earlyContinueHandler(){
      const active = document.querySelector(".language.active");
      const language = active?.dataset?.lang || localStorage.getItem("language") || "hi";
      ...
      document.documentElement.dataset.authState = "logged-out";
      ...
      // Keep the auth-only navigation hidden on the login screen.
      document.querySelectorAll(".bottom-nav, #mainApp .top-header, #sideMenu, #menuOverlay, #accountDrawer, #accountOverlay")
        .forEach((el) => el.classList.add("auth-ui-hidden"));
    }, { once: false });
  } catch (error) {
    console.error("Early language bootstrap error:", error);
  }
})();
...
/* =========================================================
   AUTH UI VISIBILITY — HARD PROTECTION
   Login screen must NEVER show the main app navigation.
========================================================= */
...
function setAuthVisibility(loggedIn) {
  ...
  const header = document.querySelector("#mainApp .top-header");
  ...
  [nav, header, menu, menuOverlay, accountDrawer, accountOverlay]
    .filter(Boolean)
    .forEach((element) => {
      element.classList.toggle("auth-ui-hidden", !loggedIn);
    });
    ...
    firebaseAuthModule.onAuthStateChanged(
      ...
      (user) => {
        ...
        if (user) {
          ...
          /* Always return to a clean login-only UI. */
          localStorage.removeItem("nakshatraUser");
...
$("logoutButton")?.addEventListener(
  ...
  async () => {
    ...
    try {
      ...
      hideAcharyaDashboard();
      currentRole = "user";
      currentProfile = null;
      currentConversation = null;
      acharyaDashboardLastUnread = new Set();
...
(() => {
  ...
  let blogCache = [];
  let aiMessages = [];
  let acharyaDashboardChatUnsubscribe = null;
  let acharyaDashboardLastUnread = new Set();
  ...
  async function resolveAcharyaRole() {
    const user = getAuthUser();
    if (!user || !firestoreReady()) return false;

    try {
      const acharyasRef = F().collection(DB(), "acharyas");
      const q = F().query(
        acharyasRef,
      ...
      if (!snap.empty) {
        ...
        const profileRef = F().doc(DB(), "users", user.uid);
        await F().setDoc(profileRef, {
          uid: user.uid,
          name: user.displayName || user.email?.split("@")[0] || snap.docs[0].data().name || "Acharya",
          email: user.email || "",
          role: "acharya",
          updatedAt: F().serverTimestamp()
        }, { merge: true });

        if (currentProfile) currentProfile.role = "acharya";
        else currentProfile = { role: "acharya" };

        return true;
      }
    } catch (error) {
      console.warn("Acharya role resolution failed:", error);
    }
  ...
  async function openConversation(conversation) {
    ...
    if (reader && conversation?.unreadForUid === reader.uid && firestoreReady()) {
      try {
        await F().updateDoc(
  ...
  async function sendChatMessage(event) {
    ...
    try {
      ...
      const participants = Array.isArray(currentConversation.participantUids)
        ? currentConversation.participantUids
        : [];
  ...
  function renderConversationList(conversations) {
    ...
    if (!conversations.length) {
      list.innerHTML = `
        <div class="message-empty premium-empty">
          ...
          <h3>${currentRole === "acharya" ? "अभी कोई user message नहीं है" : "अभी कोई निजी चैट नहीं है"}</h3>
          <p>${currentRole === "acharya" ? "जब कोई user आपको संदेश भेजेगा, बातचीत यहाँ दिखाई देगी।" : "कॉल पेज से किसी आचार्य को चुनकर संदेश शुरू करें, या AI assistant खोलें।"}</p>
          ${currentRole === "acharya" ? "" : `<button id="openAiFromEmpty" class="primary-button compact-button" type="button">✦ AI से बात करें</button>`}
        </div>`;
      $("openAiFromEmpty")?.addEventListener("click", showAIWorkspace);
      return;
    }

    const isAcharya = currentRole === "acharya";
    list.innerHTML = `
      <div class="inbox-topbar">
        <div>
          <strong>${isAcharya ? "User Messages" : "आपकी निजी बातचीत"}</strong>
          <small>${isAcharya ? "आपके users की निजी बातचीत और नए संदेश।" : "हर बातचीत अलग और सुरक्षित है।"}</small>
        </div>
        ${isAcharya ? "" : `<button id="openAiFromInbox" class="ai-inbox-button" type="button">✦ AI</button>`}
      </div>
      <div class="conversation-list">
        ${conversations.map((c) => {
          const title = isAcharya ? (c.userName || "User") : (c.acharyaName || "आचार्य");
          const unread = c.unreadForUid === getAuthUser()?.uid;
          return `
            <button class="conversation-card ${unread ? "unread" : ""}" data-open-conversation="${safeText(c.id)}" type="button">
              <div class="conversation-avatar">${safeText((title || "U").charAt(0))}</div>
              <div class="conversation-copy">
                <strong>${safeText(title)}</strong>
                <small>${safeText(c.lastMessage || "बातचीत शुरू करें")}</small>
              </div>
              <time>${safeText(formatFeatureDate(c.lastAt))}</time>
              ${unread ? `<span class="conversation-unread-dot">नया</span>` : ""}
            </button>`;
        }).join("")}
      </div>`;
    ...
    list.querySelectorAll("[data-open-conversation]").forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        if (conversation) {
          if (currentRole === "acharya") {
            await openAcharyaDashboardChat(conversation);
          } else {
  ...
  function updateMessageCount(conversations) {
    const user = getAuthUser();
    const count = conversations.filter((c) => c.unreadForUid === user?.uid).length;
  ...
  /* =========================================================
     ACHARYA DASHBOARD — PRIVATE WORKSPACE
  ========================================================= */

  function acharyaDashboardStyles() {
    if ($("acharyaDashboardStyles")) return;
    const style = document.createElement("style");
    style.id = "acharyaDashboardStyles";
    style.textContent = `
      #acharyaDashboard{position:fixed;inset:0;z-index:99990;background:var(--bg,#faf8f2);overflow:auto;font-family:inherit}
      #acharyaDashboard .ad-shell{max-width:1180px;margin:0 auto;padding:24px 18px 60px}
      #acharyaDashboard .ad-top{display:flex;align-items:center;justify-content:space-between;gap:15px;padding:14px 4px 22px}
      #acharyaDashboard .ad-brand{display:flex;align-items:center;gap:12px}
      #acharyaDashboard .ad-avatar{width:48px;height:48px;border-radius:50%;display:grid;place-items:center;background:#d8a43a;color:#17120a;font-weight:900;font-size:19px}
      #acharyaDashboard .ad-title{font-size:22px;font-weight:900;margin:0}
      #acharyaDashboard .ad-sub{font-size:13px;color:var(--muted,#777);margin-top:3px}
      #acharyaDashboard .ad-logout{border:1px solid var(--border,#e6e0d3);background:#fff;border-radius:12px;padding:10px 14px;cursor:pointer}
      #acharyaDashboard .ad-tabs{display:flex;gap:10px;flex-wrap:wrap;margin:4px 0 20px}
      #acharyaDashboard .ad-tab{border:1px solid var(--border,#e6e0d3);background:#fff;border-radius:13px;padding:11px 16px;font-weight:800;cursor:pointer}
      #acharyaDashboard .ad-tab.active{background:#d8a43a;color:#17120a;border-color:#d8a43a}
      #acharyaDashboard .ad-panel{background:#fff;border:1px solid var(--border,#e6e0d3);border-radius:22px;padding:20px;box-shadow:0 8px 30px rgba(0,0,0,.05)}
      #acharyaDashboard .ad-heading{margin:0 0 5px;font-size:25px}
      #acharyaDashboard .ad-muted{color:var(--muted,#777);font-size:14px}
      #acharyaDashboard .ad-compose{display:grid;gap:13px;margin-top:18px}
      #acharyaDashboard .ad-input,#acharyaDashboard .ad-textarea,#acharyaDashboard .ad-select{width:100%;box-sizing:border-box;border:1px solid var(--border,#e6e0d3);border-radius:12px;padding:13px;background:#fff;font:inherit}
      #acharyaDashboard .ad-textarea{min-height:230px;resize:vertical}
      #acharyaDashboard .ad-actions{display:flex;gap:10px;flex-wrap:wrap}
      #acharyaDashboard .ad-primary{border:0;background:#d8a43a;color:#17120a;border-radius:12px;padding:12px 18px;font-weight:900;cursor:pointer}
      #acharyaDashboard .ad-secondary{border:1px solid var(--border,#e6e0d3);background:#fff;border-radius:12px;padding:12px 18px;font-weight:800;cursor:pointer}
      #acharyaDashboard .ad-status{font-size:13px;color:#80601b;min-height:18px}
      #acharyaDashboard .ad-posts{display:grid;gap:10px;margin-top:18px}
      #acharyaDashboard .ad-post{display:flex;align-items:center;justify-content:space-between;gap:15px;border:1px solid var(--border,#e6e0d3);border-radius:14px;padding:14px}
      #acharyaDashboard .ad-post small{display:block;color:var(--muted,#777);margin-top:4px}
      #acharyaDashboard .ad-post-actions{display:flex;gap:7px}
      #acharyaDashboard .ad-post-actions button{border:1px solid var(--border,#e6e0d3);background:#fff;border-radius:9px;padding:8px 10px;cursor:pointer}
      #acharyaDashboard .ad-chat{margin-top:18px;border:1px solid var(--border,#e6e0d3);border-radius:18px;overflow:hidden}
      #acharyaDashboard .ad-chat-head{display:flex;align-items:center;gap:10px;padding:14px;border-bottom:1px solid var(--border,#e6e0d3);background:#fff}
      #acharyaDashboard .ad-chat-back{border:1px solid var(--border,#e6e0d3);background:#fff;border-radius:10px;padding:8px 11px;cursor:pointer}
      #acharyaDashboard .ad-chat-messages{height:52vh;overflow:auto;padding:18px;background:#faf8f2;display:grid;align-content:start;gap:9px}
      #acharyaDashboard .ad-bubble{max-width:75%;padding:10px 13px;border-radius:15px;background:#fff;border:1px solid #eee6d6;justify-self:start}
      #acharyaDashboard .ad-bubble.mine{justify-self:end;background:#f6e7bd}
      #acharyaDashboard .ad-bubble small{display:block;margin-top:4px;color:#8b836f;font-size:10px}
      #acharyaDashboard .ad-chat-form{display:flex;gap:8px;padding:12px;border-top:1px solid var(--border,#e6e0d3);background:#fff}
      #acharyaDashboard .ad-chat-form input{flex:1;border:1px solid var(--border,#e6e0d3);border-radius:11px;padding:12px;font:inherit}
      #acharyaDashboard .ad-chat-form button{border:0;background:#d8a43a;border-radius:11px;padding:0 18px;font-size:20px;cursor:pointer}
      #acharyaDashboard .ad-empty{text-align:center;padding:55px 20px;color:var(--muted,#777)}
      #acharyaDashboard .ad-unread{border-color:#d8a43a;background:#fffaf0}
      #acharyaDashboard .ad-user-name{font-weight:900}
      @media(max-width:700px){#acharyaDashboard .ad-shell{padding:14px 10px 40px}#acharyaDashboard .ad-panel{padding:14px;border-radius:17px}#acharyaDashboard .ad-top{align-items:flex-start}#acharyaDashboard .ad-title{font-size:18px}#acharyaDashboard .ad-chat-messages{height:55vh}}
    `;
    document.head.appendChild(style);
  }

  function ensureAcharyaDashboardDOM() {
    let root = $("acharyaDashboard");
    if (root) return root;
    acharyaDashboardStyles();
    root = document.createElement("section");
    root.id = "acharyaDashboard";
    root.className = "hidden";
    root.innerHTML = `
      <div class="ad-shell">
        <header class="ad-top">
          <div class="ad-brand">
            <div class="ad-avatar" id="adAvatar">आ</div>
            <div><h1 class="ad-title" id="adName">आचार्य</h1><div class="ad-sub">Acharya Workspace • निजी संदेश एवं लेख</div></div>
          </div>
          <button class="ad-logout" id="adLogout" type="button">लॉग आउट</button>
        </header>
        <nav class="ad-tabs">
          <button class="ad-tab active" data-ad-tab="messages" type="button">💬 संदेश <span id="adUnreadBadge"></span></button>
          <button class="ad-tab" data-ad-tab="posts" type="button">📝 मेरे लेख</button>
        </nav>
        <div id="adMessagesPanel" class="ad-panel"></div>
        <div id="adPostsPanel" class="ad-panel hidden"></div>
      </div>`;
    document.body.appendChild(root);

    root.querySelectorAll("[data-ad-tab]").forEach((tab) => {
      tab.addEventListener("click", async () => {
        root.querySelectorAll("[data-ad-tab]").forEach((x) => x.classList.remove("active"));
        tab.classList.add("active");
        const mode = tab.dataset.adTab;
        $("adMessagesPanel")?.classList.toggle("hidden", mode !== "messages");
        $("adPostsPanel")?.classList.toggle("hidden", mode !== "posts");
        if (mode === "messages") await renderAcharyaDashboardMessages();
        if (mode === "posts") await renderAcharyaDashboardPosts();
      });
    });
    $("adLogout")?.addEventListener("click", () => $("logoutButton")?.click());
    return root;
  }

  function showAcharyaDashboard() {
    const root = ensureAcharyaDashboardDOM();
    root.classList.remove("hidden");
    hide(mainApp);
    const user = getAuthUser();
    const profileName = currentProfile?.name || user?.displayName || user?.email?.split("@")[0] || "आचार्य";
    $("adName").textContent = profileName;
    $("adAvatar").textContent = profileName.charAt(0).toUpperCase();
  }

  function hideAcharyaDashboard() {
    const root = $("acharyaDashboard");
    if (root) root.classList.add("hidden");
    show(mainApp);
    if (acharyaDashboardChatUnsubscribe) {
      acharyaDashboardChatUnsubscribe();
      acharyaDashboardChatUnsubscribe = null;
    }
  }

  async function renderAcharyaDashboard() {
    if (currentRole !== "acharya") return;
    showAcharyaDashboard();
    await renderAcharyaDashboardMessages();
    await renderAcharyaDashboardPosts();
    requestAcharyaNotificationPermission();
  }

  async function renderAcharyaDashboardMessages() {
    const panel = $("adMessagesPanel");
    const user = getAuthUser();
    if (!panel || !user || !firestoreReady()) return;
    ...
    panel.innerHTML = `
      <h2 class="ad-heading">Users के संदेश</h2>
      <p class="ad-muted">जिस user ने आपको message किया है, उसकी निजी बातचीत यहाँ दिखेगी।</p>
      <div id="adConversationList" class="ad-posts"><div class="ad-empty">संदेश लोड हो रहे हैं…</div></div>
      <div id="adChatBox"></div>`;

    const q = F().query(
      F().collection(DB(), "conversations"),
      F().where("participantUids", "array-contains", user.uid),
      F().limit(100)
    );

    acharyaDashboardChatUnsubscribe = F().onSnapshot(q, async (snap) => {
      const conversations = snap.docs.map((d) => ({ id: d.id, ...d.data() }))
        .filter((c) => c.acharyaUid === user.uid || c.participantUids?.includes(user.uid))
        .sort((a,b) => timestampMs(b.lastAt) - timestampMs(a.lastAt));
      renderAcharyaConversationCards(conversations);
      const unread = conversations.filter(c => c.unreadForUid === user.uid);
      $("adUnreadBadge").textContent = unread.length ? `(${unread.length})` : "";
      if (unread.length) {
        const fresh = unread.filter(c => !acharyaDashboardLastUnread.has(c.id));
        if (fresh.length) showAcharyaNotifications(fresh);
      }
      acharyaDashboardLastUnread = new Set(unread.map(c => c.id));
    }, (error) => {
      console.error("Acharya inbox error:", error);
      const box = $("adConversationList");
      if (box) box.innerHTML = `<div class="ad-empty">Messages load नहीं हो सके। Firestore rules जाँचें।</div>`;
    });
  }

  function renderAcharyaConversationCards(conversations) {
    const box = $("adConversationList");
    if (!box) return;
    if (!conversations.length) {
      box.innerHTML = `<div class="ad-empty">अभी कोई user message नहीं आया है।</div>`;
      return;
    }
    const uid = getAuthUser()?.uid;
    box.innerHTML = conversations.map(c => {
      const unread = c.unreadForUid === uid;
      return `<button type="button" class="conversation-card ${unread ? "ad-unread" : ""}" data-ad-open-chat="${safeText(c.id)}" style="width:100%;text-align:left;display:flex;align-items:center;gap:12px;border:1px solid var(--border,#e6e0d3);border-radius:14px;background:#fff;padding:13px;cursor:pointer">
        <div class="conversation-avatar">${safeText((c.userName || "U").charAt(0))}</div>
        <div class="conversation-copy" style="flex:1"><strong class="ad-user-name">${safeText(c.userName || "User")}</strong><small style="display:block;color:var(--muted,#777);margin-top:4px">${safeText(c.lastMessage || "नया संवाद")}</small></div>
        <time style="font-size:11px;color:#8b836f">${safeText(formatFeatureDate(c.lastAt))}</time>
        ${unread ? `<b style="font-size:11px;color:#9a7015">नया</b>` : ""}
      </button>`;
    }).join("");
    box.querySelectorAll("[data-ad-open-chat]").forEach(btn => btn.addEventListener("click", async () => {
      const c = conversations.find(x => x.id === btn.dataset.adOpenChat);
      if (c) await openAcharyaDashboardChat(c);
    }));
  }

  async function openAcharyaDashboardChat(conversation) {
    const box = $("adChatBox");
    const user = getAuthUser();
    if (!box || !user || !firestoreReady()) return;
    try {
      if (conversation.unreadForUid === user.uid) {
        await F().updateDoc(F().doc(DB(), "conversations", conversation.id), { unreadForUid: "" });
      }
    } catch (e) { console.warn("Read status update failed", e); }
    if (acharyaDashboardChatUnsubscribe) {
      acharyaDashboardChatUnsubscribe();
      acharyaDashboardChatUnsubscribe = null;
    }
    box.innerHTML = `
      <div class="ad-chat">
        <div class="ad-chat-head"><button class="ad-chat-back" id="adChatBack" type="button">←</button><div><strong>${safeText(conversation.userName || "User")}</strong><div class="ad-sub">निजी बातचीत</div></div></div>
        <div id="adChatMessages" class="ad-chat-messages"></div>
        <form id="adChatForm" class="ad-chat-form"><input id="adChatInput" placeholder="उत्तर लिखें…" autocomplete="off"><button type="submit">➤</button></form>
      </div>`;
    $("adChatBack")?.addEventListener("click", () => { box.innerHTML = ""; renderAcharyaDashboardMessages(); });
    $("adChatForm")?.addEventListener("submit", async (event) => {
      event.preventDefault();
      const input = $("adChatInput");
      const text = input?.value.trim();
      if (!text) return;
      input.value = "";
      try {
        await F().addDoc(F().collection(DB(), "conversations", conversation.id, "messages"), {
          senderUid: user.uid,
          senderRole: "acharya",
          text,
          createdAt: F().serverTimestamp()
        });
        await F().updateDoc(F().doc(DB(), "conversations", conversation.id), {
          lastMessage: text.slice(0, 180),
          lastAt: F().serverTimestamp(),
          lastSenderUid: user.uid,
          unreadForUid: conversation.userId || (conversation.participantUids || []).find(x => x !== user.uid) || ""
        });
      } catch (error) {
        console.error("Acharya reply error:", error);
        showFeatureToast("उत्तर भेजा नहीं जा सका।");
      }
    });
    const messagesRef = F().collection(DB(), "conversations", conversation.id, "messages");
    const q = F().query(messagesRef, F().orderBy("createdAt", "asc"));
    acharyaDashboardChatUnsubscribe = F().onSnapshot(q, snap => {
      const messages = snap.docs.map(d => ({id:d.id,...d.data()}));
      const messagesBox = $("adChatMessages");
      if (!messagesBox) return;
      messagesBox.innerHTML = messages.length ? messages.map(m => `<div class="ad-bubble ${m.senderUid === user.uid ? "mine" : ""}"><div>${safeText(m.text).replaceAll("\n","<br>")}</div><small>${safeText(formatFeatureDate(m.createdAt))}</small></div>`).join("") : `<div class="ad-empty">अभी कोई message नहीं है।</div>`;
      messagesBox.scrollTop = messagesBox.scrollHeight;
    }, error => console.error("Acharya chat listener error:", error));
  }

  async function renderAcharyaDashboardPosts() {
    const panel = $("adPostsPanel");
    const user = getAuthUser();
    if (!panel || !user || currentRole !== "acharya" || !firestoreReady()) return;
    let posts = [];
    try {
      const snap = await F().getDocs(F().query(F().collection(DB(), "posts"), F().where("authorUid", "==", user.uid), F().limit(100)));
      posts = snap.docs.map(d => ({id:d.id,...d.data()})).sort((a,b)=>timestampMs(b.updatedAt||b.createdAt)-timestampMs(a.updatedAt||a.createdAt));
    } catch (error) {
      console.error("Acharya posts load error:", error);
    }
    panel.innerHTML = `
      <h2 class="ad-heading">मेरे लेख</h2>
      <p class="ad-muted">आप अपना लेख लिखकर प्रकाशित कर सकते हैं। केवल आपके लेख यहाँ manage होंगे।</p>
      <form id="acharyaPostForm" class="ad-compose">
        <input id="acharyaPostId" type="hidden">
        <input id="acharyaPostTitle" class="ad-input" placeholder="लेख का शीर्षक" required>
        <select id="acharyaPostCategory" class="ad-select"><option value="jyotish">ज्योतिष</option><option value="kundli">कुंडली</option><option value="muhurat">मुहूर्त</option><option value="guidance">मार्गदर्शन</option><option value="other">अन्य</option></select>
        <input id="acharyaPostExcerpt" class="ad-input" placeholder="छोटा परिचय / सारांश">
        <input id="acharyaPostCoverUrl" class="ad-input" placeholder="Cover image URL (वैकल्पिक)">
        <textarea id="acharyaPostContent" class="ad-textarea" placeholder="पूरा लेख यहाँ लिखें…" required></textarea>
        <label style="display:flex;gap:8px;align-items:center;font-weight:800"><input id="acharyaPostPublished" type="checkbox" checked> अभी प्रकाशित करें</label>
        <div class="ad-actions"><button class="ad-primary" type="submit">लेख सुरक्षित करें</button><button class="ad-secondary" id="acharyaPostReset" type="button">नया लेख</button></div>
        <div id="acharyaPostStatus" class="ad-status"></div>
      </form>
      <div class="ad-posts" id="acharyaPostList"></div>`;
    const list = $("acharyaPostList");
    list.innerHTML = posts.length ? posts.map(post => `<article class="ad-post"><div><strong>${safeText(post.title)}</strong><small>${safeText(blogCategoryName(post.category))} • ${post.published ? "Published" : "Draft"}</small></div><div class="ad-post-actions"><button type="button" data-ad-edit-post="${safeText(post.id)}">संपादित</button><button type="button" data-ad-delete-post="${safeText(post.id)}">हटाएँ</button></div></article>`).join("") : `<div class="ad-empty">अभी आपने कोई लेख नहीं लिखा है।</div>`;
    $("acharyaPostForm")?.addEventListener("submit", saveAcharyaDashboardPost);
    $("acharyaPostReset")?.addEventListener("click", resetAcharyaDashboardPostForm);
    list.querySelectorAll("[data-ad-edit-post]").forEach(btn => btn.addEventListener("click", () => {
      const post = posts.find(x => x.id === btn.dataset.adEditPost); if (!post) return;
      $("acharyaPostId").value = post.id; $("acharyaPostTitle").value = post.title || ""; $("acharyaPostCategory").value = post.category || "other"; $("acharyaPostExcerpt").value = post.excerpt || ""; $("acharyaPostCoverUrl").value = post.coverUrl || ""; $("acharyaPostContent").value = post.content || ""; $("acharyaPostPublished").checked = post.published !== false; window.scrollTo({top:0,behavior:"smooth"});
    }));
    list.querySelectorAll("[data-ad-delete-post]").forEach(btn => btn.addEventListener("click", async () => {
      if (!window.confirm("क्या आप इस लेख को हटाना चाहते हैं?")) return;
      try { await F().deleteDoc(F().doc(DB(), "posts", btn.dataset.adDeletePost)); await loadBlog(); await renderAcharyaDashboardPosts(); } catch(e) { console.error(e); showFeatureToast("लेख हटाया नहीं जा सका।"); }
    }));
  }

  async function saveAcharyaDashboardPost(event) {
    event.preventDefault();
    const user = getAuthUser();
    if (!user || currentRole !== "acharya" || !firestoreReady()) return;
    const title = $("acharyaPostTitle").value.trim();
    const content = $("acharyaPostContent").value.trim();
    const status = $("acharyaPostStatus");
    if (!title || content.length < 20) { status.textContent = "शीर्षक और कम से कम 20 अक्षरों का लेख आवश्यक है।"; return; }
    const payload = {title, excerpt:$("acharyaPostExcerpt").value.trim(), content, category:$("acharyaPostCategory").value, coverUrl:$("acharyaPostCoverUrl").value.trim(), published:$("acharyaPostPublished").checked, authorUid:user.uid, authorName:user.displayName || user.email?.split("@")[0] || currentProfile?.name || "Acharya", updatedAt:F().serverTimestamp()};
    try {
      const id = $("acharyaPostId").value.trim();
      if (id) await F().updateDoc(F().doc(DB(), "posts", id), payload);
      else await F().addDoc(F().collection(DB(), "posts"), {...payload, createdAt:F().serverTimestamp()});
      status.textContent = "लेख सफलतापूर्वक सुरक्षित हो गया।";
      await loadBlog();
      await renderAcharyaDashboardPosts();
    } catch (error) { console.error("Acharya post save error:", error); status.textContent = "लेख सुरक्षित नहीं हो सका। Firestore rules जाँचें।"; }
  }

  function resetAcharyaDashboardPostForm() {
    $("acharyaPostForm")?.reset();
    if ($("acharyaPostId")) $("acharyaPostId").value = "";
    if ($("acharyaPostPublished")) $("acharyaPostPublished").checked = true;
  }

  async function requestAcharyaNotificationPermission() {
  ...
  function showAcharyaNotifications(conversations) {
    if (typeof Notification === "undefined" || Notification.permission !== "granted") return;
    conversations.forEach(c => {
      try { new Notification(`नया संदेश — ${c.userName || "User"}`, { body: c.lastMessage || "आपको नया संदेश मिला है।", tag: `nakshatra-${c.id}` }); } catch (e) {}
    });
  }
