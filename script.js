...
function restoreLastPage() {
  ...
  /* Messages has two internal views: Inbox and AI.
     Restore the correct one instead of showing the page's
     static/default markup after a browser refresh. */
  if (lastPage === "messages") {
    ...
    setTimeout(() => {
      ...
      if (lastMessagesView === "ai") {

        if (typeof showAIWorkspace === "function") {
          showAIWorkspace();
        }
      ...
      if (typeof showMessagesInbox === "function") {
        showMessagesInbox();
      }

      if (typeof loadMessagesInbox === "function") {
        loadMessagesInbox();
      }
...
(() => {
      ...
      <article class="acharya-detail" data-acharya-id="${safeText(a.id)}">
        ...
        <div class="acharya-detail-content">
          ...
          <div class="social-buttons">
            <button type="button" data-acharya-chat="${safeText(a.id)}">💬 संदेश</button>
            <button type="button" data-acharya-call="${safeText(a.id)}">☎ कॉल</button>
          </div>
        </div>
      </article>
    `).join("");

    list.querySelectorAll("[data-acharya-chat]").forEach((button) => {
      button.addEventListener("click", async () => {
        await openAcharyaChat(await getAcharyaById(button.dataset.acharyaChat));
      });
    });

    list.querySelectorAll("[data-acharya-call]").forEach((button) => {
      button.addEventListener("click", async () => {
        startAcharyaCall(await getAcharyaById(button.dataset.acharyaCall));
      });
    });
  }
  ...
  function showAIWorkspace() {
    /* Persist the active Messages sub-view so a refresh does not
       fall back to the static/default Call page. */
    localStorage.setItem("lastPage", "messages");
    localStorage.setItem("lastMessagesView", "ai");
  ...
  function wireEvents() {
    ...
    document.querySelectorAll('[data-page="messages"]').forEach((button) => {
      button.addEventListener("click", () => {
        localStorage.setItem("lastMessagesView", "inbox");
        setTimeout(() => {
          showMessagesInbox();
          loadMessagesInbox();
        }, 120);
  ...
  /* Restore the page/view that was active before refresh. */
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", restoreLastPage, { once: true });
  } else {
    restoreLastPage();
  }
